...
pdf("DMS113_network_means_PCC.pdf", height=5, width=8)
pcc_m <- plot(NetMean, "PCC", labels= vars_m, edge.labels = TRUE, layout="spring")
dev.off()

pdf("DMS113_network_means_PDC.pdf", height=5, width=8)
pdc_m <- plot(NetMean, "PDC", labels = vars_m)
dev.off()

# create and save centrality plots
DMS113_centplotm <- plot(NetMean, labels= vars_m, edge.labels = TRUE, layout="spring") %>% 
  centralityPlot() +
  theme(legend.position = "none") +
  facet_grid(rows = vars(measure)) +
  theme(axis.text=element_text(size=13),
        strip.text.y = element_text(size=15))
ggsave(DMS113_centplotm, filename = "DMS113_centrality_m.png")

# centrality tables
centr_m<-gather_cent(pcc_m, pdc_m)
rm(centr)
write.csv(centr_m, "DMS113_centr_means.csv", row.names = FALSE)

# node correlations. Get edge list and weights
pcc_corr_m <- data.frame(pcc_m[[1]][[2]][[6]])
pcc_corr_m # look at it
write.csv(pcc_corr_m, "DMS113_pcc_corr_means.csv", row.names = TRUE)

pdc_corr_m <- data.frame(pdc_m[[1]][[2]][[5]])
pdc_corr_m # look at it
write.csv(pdc_corr_m, "DMS113_pdc_corr_means.csv", row.names = TRUE)
```
...
# save graphs
pdf("DMS113_network_sd_PCC.pdf", height=5, width=8)
pcc_sd <- plot(NetSD, "PCC", labels= vars_sd, edge.labels = TRUE, layout="spring")
dev.off()

pdf("DMS113_network_sd_PDC.pdf", height=5, width=8)
pdc_sd <- plot(NetSD, "PDC", labels = vars_sd)
dev.off()

# create and save centrality plots
DMS113_centplotsd <- plot(NetSD, labels= vars_sd, edge.labels = TRUE, layout="spring") %>% 
  centralityPlot() +
  theme(legend.position = "none") +
  facet_grid(rows = vars(measure)) +
  theme(axis.text=element_text(size=13),
        strip.text.y = element_text(size=15))
ggsave(DMS113_centplotsd, filename = "DMS113_centrality_sd.png")

# centrality tables
centr_sd <- gather_cent(pcc_sd, pdc_sd)
rm(centr)
write.csv(centr_sd, "DMS113_centr_sd.csv", row.names = FALSE)

# node correlations
pcc_corr_sd <- data.frame(pcc_sd[[1]][[2]][[6]])
pcc_corr_sd # look at it
write.csv(pcc_corr_sd, "DMS113_pcc_corr_sd.csv", row.names = TRUE)

pdc_corr_sd <- data.frame(pdc_sd[[1]][[2]][[5]])
pdc_corr_sd # look at it
write.csv(pdc_corr_sd, "DMS113_pdc_corr_sd.csv", row.names = TRUE)
```
